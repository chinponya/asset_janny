import { Resource, toBasePath } from "./resources.ts";
...
function buildTitleMapping(
  ...
  if (english_name && !metadataHasForeignName(metadata)) {
    new_path = base_path + stripIllegalCharacters(english_name) + suffix +
      extension;
  } else {
...
function buildTitleMappings(metadata: Metadata): Mappings {
  ...
  for (const entry of metadata["item_definition_title"] || []) {
    ...
    const [item_path, new_item_path] = buildTitleMapping(
      entry,
      "icon_item",
      ".jpg",
      " item",
...
function buildItemMappings(metadata: Metadata, key: string): Mappings {
  ...
  for (const entry of metadata[key] || []) {
    ...
    if (english_name && !metadataHasForeignName(entry)) {
      mappings[file_path] = base_path + stripIllegalCharacters(english_name) +
        suffix + extension;
    } else {
...
function buildRankMappings(metadata: Metadata): Mappings {
  ...
  for (const entry of metadata["level_definition_level_definition"] || []) {
    ...
    mappings[file_path] = base_path + prefix +
      stripIllegalCharacters(english_name) + extension;
  }

  return mappings;
}

function buildSkinMappings(
  metadata: Metadata,
  character_id_mappings: CharacterIdMappings,
): Mappings {
  const mappings: Record<string, string> = {};
...
function buildEmoteMappings(metadata: Metadata): Mappings {
  ...
  for (const character of metadata["item_definition_character"] || []) {
    mappings[character["emo"]] = base_path +
      stripIllegalCharacters(character["name_en"]);
  }
  return mappings;
}
...
export function mapPath(resource: Resource, mappings: Mappings): string {
  const base_path = toBasePath(resource);
