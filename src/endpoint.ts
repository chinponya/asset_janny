...
import { parseVersion, Version, versionToString } from "./version.ts";
import {
  compareByLanguage,
  findConfigProtoResource,
  findMetadataResource,
  fromResversion,
  Language,
  Resource,
  Resources,
} from "./resources.ts";
import { Decryptor, isEncrypted } from "./xor.ts";
import { protobufjs, Random, streams } from "./deps.ts";
...
const EXISTS_AT_ROOT = new RegExp(
  "^(index.html|close.html|background.jpg|favicon.ico|favicon_en.ico|icon.png|icon_en.png|client_language.txt|sw.js|alitrace.js)$",
);
...
export function gameServerUrl(game_server: GameServer): URL {
  switch (game_server) {
    ...
    default:
      return gameServerUrl(defaultGameServer);
  }
}
...
export async function fetchResversion(
  server: GameServer,
  version: Version,
...
export async function fetchConfigProto(
  resources: Resources,
): Promise<protobufjs.Root> {
...
function parseCfHeader(header: string): Record<string, string> {
  return header
    ...
    .reduce((acc: Record<string, string>, values) => {
      acc[decodeURIComponent(values[0].trim())] = decodeURIComponent(
        values[1].trim(),
      );
      return acc;
    }, {});
